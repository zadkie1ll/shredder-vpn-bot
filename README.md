...
`/referrals <tgid>` показывает реферальную статистику выбранного пользователя:
приглашённых, конверсию в оплату, суммы оплат и бонусные дни.
В списке показаны последние 30 приглашённых; итоговые числа учитывают всех.
Старые команды `/refs` и `/ref-stats` также работают.

`/notifications_report` отправляет текущую статистику уведомлений только в чат
`1297686797`. Ежедневный отчёт в 19:00 МСК отправляется туда же, независимо
от списка администраторов. Обе команды доступны только администраторам.
