...
Перед тестом подставить свой TG ID вместо `123456789`.

1. Проверить обычный кнопочный feedback без дополнительных уточнений:

```text
/feedback_test 123456789 buttons month:40
```

Проверить кнопки:

```text
Не устроила цена - сразу выдает скидку.
Не разобрался с подключением - без флага сразу выдает скидку.
Не хватило скорости - сразу выдает скидку.
Не было нужной локации - без флага сразу выдает скидку.
Другая причина - всегда просит текст, потом выдает скидку.
```

2. Проверить запрос страны для кнопки `Не было нужной локации`:

```text
/feedback_test 123456789 buttons month:40 --ask-location
```

Нажать `Не было нужной локации`, написать страну, убедиться, что после текста
появилась скидка.

3. Проверить поддержку для кнопки `Не разобрался с подключением`:

```text
/feedback_test 123456789 buttons month:40 --connection-support
```

Нажать `Не разобрался с подключением`, убедиться, что бот показал контакт
поддержки и затем выдал скидку.

4. Проверить оба флага вместе:

```text
/feedback_test 123456789 buttons month:40,year:30 --ask-location --connection-support
```
...
Проверить список TG ID и нажать `Отмена`, если рассылку запускать не нужно.
